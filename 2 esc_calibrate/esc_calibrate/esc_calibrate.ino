...
int loop_counter, MPU_ADDRESS, vibration_counter;
int temperature;
...
void setup(){
	...
	for ( data = 0; data <= 35; data++ ) {												//Read EEPROM for faster data access
		eeprom_data[data] = EEPROM.read(data);
	}

	MPU_ADDRESS = eeprom_data[32];                                                       //Store the gyro address in the variable.

	setMPUConfig();                                                                 //Set the specific gyro registers.
...
void gyro_signalen(){
	...
	if(eeprom_data[31] == 1){
		...
		Wire.write(0x3B);                                            //Start reading @ register 43h and auto increment with every read.
		Wire.endTransmission();                                      //End the transmission.
		Wire.requestFrom(MPU_ADDRESS,14);                           //Request 14 bytes from the gyro.
